...
if "%FILEID%"=="0" (
  ...
  set "CUSTOM_FILENAME=%MODEL_NAME%-%MAINTAINER%-BF16-SPECIAL_TENSOR-00001-of-%CHUNKS_PAD%.gguf.sig"
) else (
  set "IDX=%FILEID%"
  call :padnum IDX 5
  set "FILENAME=%MODEL_NAME%-%MAINTAINER%-%QUANT_U%-SPECIAL_TENSOR-!IDX!-of-%CHUNKS_PAD%.gguf"
  set "CUSTOM_FILENAME=%MODEL_NAME%-%MAINTAINER%-BF16-SPECIAL_TENSOR-!IDX!-of-%CHUNKS_PAD%.gguf"
)
